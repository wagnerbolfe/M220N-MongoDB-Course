...
namespace M220N.Repositories
{
    public class UsersRepository
    {
        ...
        */
        ...
        {
            await _sessionsCollection.Find(new BsonDocument("user_id", email)).FirstOrDefaultAsync(cancellationToken: cancellationToken);
        ...
        {
            var session = await _sessionsCollection.Find(new BsonDocument("user_id", email)).FirstOrDefaultAsync(cancellationToken: cancellationToken);
            return session;
        }
            ...
            Dictionary<string, string> preferences, CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                var updateFields = Builders<User>.Update.Set("preferences", preferences);
